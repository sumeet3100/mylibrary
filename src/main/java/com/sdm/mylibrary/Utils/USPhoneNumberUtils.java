 ...
 */
public class USPhoneNumberUtils {
...
//            return TelephonyManager.getDefault().getVoiceMailNumber();
    ...
    public static boolean isWellFormedSmsAddress(String address) {
        String networkPortion =
                USPhoneNumberUtils.extractNetworkPortion(address);
    ...
    private static final int NANP_STATE_BRACKET = 5;
    private static final int NANP_STATE_HYP =6;
    private static final int NANP_STATE_SPACE =7;
...
//                        state = NANP_STATE_ONE;
                ...
                case '0':
                    if (state == NANP_STATE_PLUS) {
                        ...
                        // Sumeet change values from 3 to 5 6 to 10
                    } else if (state != NANP_STATE_DASH && (numDigits == 0)) {
                        // Found a digit that should be after a dash that isn't
                        dashPositions[numDashes++] = i;
                    }else if (state != NANP_STATE_BRACKET && (numDigits == 3)) {
                        // Found a digit that should be after a dash that isn't
                        dashPositions[numDashes++] = i;

                    }
//                    else if (state != NANP_STATE_SPACE && (numDigits == 3)) {
//                        // Found a digit that should be after a dash that isn't
//                        dashPositions[numDashes++] = i;
//                    }

                    else if (state != NANP_STATE_HYP && (numDigits == 5)) {
                        // Found a digit that should be after a dash that isn't
                        dashPositions[numDashes++] = i;
                    }
                ...
                case '-':
                    state = NANP_STATE_HYP;
                    break;

                case ')':
                    state = NANP_STATE_BRACKET;
                    break;
        ...
        if (numDigits >=7) {
            // With 7 digits we want xxx-xxxx, not xxx-xxx-xxxx
            numDashes--;
        }
